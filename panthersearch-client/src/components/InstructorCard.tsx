...
export default function InstructorCard({ instructor, selected = false, semesters = [], onSelect, condensed = false }: InstructorCardProps) {
  return (
    <button
      type="button"
      onClick={onSelect}
      className={`surface-card w-full cursor-pointer p-4 text-left transition hover:bg-[#EEF2FF] ${selected ? 'border-[#1B3BFF] border-l-4' : ''}`}
    >
      <div className="mb-1 text-base font-semibold text-[#111827]">{instructor.name}</div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-3 text-sm">
          ...
          <MetricBar value={instructor.rating} color="#1B3BFF" />
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          <MetricBar value={instructor.difficulty} color="#6C2DFF" />
